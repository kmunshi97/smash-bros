...
- **Procedural court (M2 court rework)**: the baked `stadium_floor.png` + `stadium_net.png` are replaced by code-drawn layers stacked over a swappable floor — `ArenaTheme` (floor colours = the only thing that changes per arena), pure `CourtGeometry` (perspective math reusing the live `CourtProjection`, so the floor/lines/net stay pixel-aligned with players + shuttle), `ArenaCourtComponent` (textured + lit perspective floor + white lines at exact engine-derived positions), and a procedural `NetComponent` (posts + mesh ribbon + cord + floor shadow, shaded). "Change the floor to change the arena." Geometry is unit-tested; `clayCourt` ships as a second theme to prove the swap.
