...
- **M2-001..005** `[S]`: sprites (8 states), parallax, camera + shake, particles, animation state machine (unchanged from v2).
  ...
  - **M2-001 (sprites, 8 states): still open** — needs real per-state sprite-sheet art. The `PlayerAnimator` already exposes the state and a 0..1 swing progress, so a future sprite-sheet renderer can swap frames off the same signals with no state-machine changes.
